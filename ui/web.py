...
def show_stock_analysis_page():
            ...
            try:
                ...
                if not stock_data.empty:
                    ...
                    if stock_info:
                        ...
                        info_cols.metric("股票名称", stock_info.get("股票简称", "未知"))
                        info_cols.metric("行业", stock_info.get("行业", "未知"))
                        info_cols.metric("总市值", stock_info.get("总市值", "未知"))
                        info_cols.metric("流通市值", stock_info.get("流通市值", "未知"))
                else:
    ...
    if st.session_state.stock_data is not None and not st.session_state.stock_data.empty:
        ...
        metrics_cols.metric("当前价格", f"¥{latest_data['close']:.2f}")
        metrics_cols.metric("开盘价", f"¥{latest_data['open']:.2f}")
        metrics_cols.metric("最高价", f"¥{latest_data['high']:.2f}")
        metrics_cols.metric("最低价", f"¥{latest_data['low']:.2f}")
        metrics_cols.metric("成交量", f"{latest_data['volume']:,}")
        ...
        with st.spinner(f"正在使用 {model_type.upper()} 模型进行预测..."):
            try:
                ...
                if "error" not in result:
                    ...
                    pred_cols.metric("当前价格", f"¥{result['current_price']:.2f}")
                    pred_cols.metric("预测价格", f"¥{result['predicted_price']:.2f}")
                    pred_cols.metric(
                        "价格变化",
        ...
        with st.spinner("正在进行风险评估..."):
            try:
                ...
                if "error" not in result:
                    ...
                    risk_cols.metric("波动率", f"{result['volatility']:.4f}")
                    risk_cols.metric("历史VaR(95%)", f"{result['var_historical']:.4f}")
                    risk_cols.metric("最大回撤", f"{result['max_drawdown']:.4f}")
                    risk_cols.metric("夏普比率", f"{result['sharpe_ratio']:.4f}")
                    
                    # Additional metrics
                    st.markdown("<h3 class='subsection-header'>更多指标</h3>", unsafe_allow_html=True)
                    more_cols = st.columns(4)
                    more_cols.metric("贝塔系数", f"{result['beta']:.4f}")
                    more_cols.metric("Alpha值", f"{result['alpha']:.4f}")
                    more_cols.metric("市场相关性", f"{result['correlation_with_market']:.4f}")
                    more_cols.metric("数据点数", result['data_points'])
                    ...
                    mc_cols.metric("预期损失", f"{mc_results['expected_loss']:.4f}")
                    mc_cols.metric("VaR 95%", f"{mc_results['var_95']:.4f}")
                    mc_cols.metric("VaR 99%", f"{mc_results['var_99']:.4f}")
                    mc_cols.metric("最小损失", f"{mc_results['min_loss']:.4f}")
                    mc_cols.metric("最大损失", f"{mc_results['max_loss']:.4f}")
            ...
            with st.spinner("正在分析投资组合..."):
                try:
                    ...
                    if "error" not in result and result.get("success"):
                        ...
                        metric_cols.metric("预期收益", f"{metrics['expected_return']:.4f}")
                        metric_cols.metric("风险(波动率)", f"{metrics['volatility']:.4f}")
                        metric_cols.metric("夏普比率", f"{metrics['sharpe_ratio']:.4f}")
            ...
            with st.spinner("正在优化投资组合..."):
                try:
                    ...
                    if "error" not in result and result.get("success"):
                        ...
                        opt_cols.metric("优化后预期收益", f"{result['expected_return']:.4f}")
                        opt_cols.metric("优化后风险", f"{result['volatility']:.4f}")
                        opt_cols.metric("优化后夏普比率", f"{result['sharpe_ratio']:.4f}")
            ...
            with st.spinner("正在进行蒙特卡洛模拟..."):
                try:
                    ...
                    if "error" not in result:
                        ...
                        sim_cols.metric("最大夏普比率", f"{result['max_sharpe_ratio']:.4f}")
                        sim_cols.metric("最小波动率", f"{result['min_volatility']:.4f}")
                        sim_cols.metric("模拟次数", result['n_simulations'])
        ...
        with st.spinner("正在进行策略回测..."):
            try:
                ...
                if "error" not in result and result.get("success"):
                    ...
                    metric_cols.metric("累计收益", f"{metrics.get('cumulative_return', 0)*100:.2f}%")
                    metric_cols.metric("年化收益", f"{metrics.get('annualized_return', 0)*100:.2f}%")
                    metric_cols.metric("夏普比率", f"{metrics.get('sharpe_ratio', 0):.2f}")
                    metric_cols.metric("最大回撤", f"{metrics.get('max_drawdown', 0)*100:.2f}%")
                    
                    # Additional metrics
                    st.markdown("<h3 class='subsection-header'>更多指标</h3>", unsafe_allow_html=True)
                    more_cols = st.columns(4)
                    more_cols.metric("交易次数", len(result["result"]["engine"].trades))
                    more_cols.metric("胜率", f"{metrics.get('win_rate', 0)*100:.2f}%")
                    more_cols.metric("盈亏比", f"{metrics.get('profit_loss_ratio', 0):.2f}")
                    more_cols.metric("最大单笔收益", f"{metrics.get('max_trade_return', 0)*100:.2f}%")
